...
philosopher(void *num)
{
  ...
  while ((f = food_on_table()) > 0)
  {
    ...
    /* Add global fork order to break circular wait and avoid deadlock. */
    int first_fork = left_fork < right_fork ? left_fork : right_fork;
    int second_fork = left_fork < right_fork ? right_fork : left_fork;
    char *first_hand = first_fork == left_fork ? "left " : "right";
    char *second_hand = second_fork == left_fork ? "left " : "right";
