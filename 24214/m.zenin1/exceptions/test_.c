...
# 4 "test.c"
enum exception { NO_EXC, NULLEXC };

int foo(jmp_buf exc_handler_up, int i) {
 jmp_buf exc_handler, default_handler; jmp_buf *curr_handler; int exc; exc = 
# 7 "test.c" 3 4
_setjmp (
# 7 "test.c"
default_handler
...
# 7 "test.c"
; if (exc != 0){ longjmp(exc_handler_up, exc); } curr_handler = &default_handler;

 if (i == 0){
  longjmp(exc_handler_up, NULLEXC);
 }
...
int main() {
 ...
 jmp_buf exc_handler, default_handler;
