...
#define CANCELLATION_CHECK_ITERATIONS 65536
...
bool finish_computing = false;

void sigint_handler(int sig) {
    finish_computing = true;
}
...
void* worker_thread_routine(void *arg) {
    ...
    while (true) {
        for (size_t i = 1; i < CANCELLATION_CHECK_ITERATIONS; i++){
...
int main(int argc, char **argv) {
    struct sigaction act = {.sa_handler = sigint_handler, .sa_flags = 0};
